...
for tissue in Tissues:
    ...
    seed_folder = '../Centrality Computation/' + tissue
    # Set the seed -- this seed is same as the seed generated while creating the population
    with open(seed_folder + '/seed.txt', 'r') as seed_file:
        seed = int(seed_file.read())
    seed_file.close()
    random.seed(seed)
    
    orig_file = '../Original Dataset/Preprocessed Files/' + tissue + '/genes.csv'
    genes = pd.read_csv(orig_file, header=0, index_col=0)
