...
ss_res_1 <-  as.data.frame(enrichment_results[,4])
row.names(ss_res_1) <- enrichment_results$description
colnames(ss_res_1) <- Tissues[i]

# i = 2
# setwd(paste0("D:/work/Sugyani_work/New_Hmps_ranks/kegg_pathways_deg_centrality/",Tissues[i],"/Project_mu_sigma/"))
# enrichment_results <- read.delim("enrichment_results_mu_sigma.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
# 
# ss_res_2 <-  as.data.frame(enrichment_results[,7])
# row.names(ss_res_2) <- enrichment_results$description
# colnames(ss_res_2) <- Tissues[i]

#ddff <- merge(ss_res_1, ss_res_2, by = "row.names", all = TRUE)
...
for(i in 2:length(Tissues)){
  ...
  ss_res_3 <- as.data.frame(enrichment_results[,4])
